"""ManufacturerColors.bin / JSON: stock PaintColorColorParam swatches.

FH6 ships ManufacturerColors.bin inside each car zip. The CLI Toolkit also emits a JSON
mirror. Prefer parsing the JSON when present next to the dump. Stock paint is the first
swatch that carries PaintColorColorParam (hash 0xC0CB2820).
"""
...
def stock_paint_rgba(swatches: list[PaintSwatch]) -> tuple[float, float, float, float] | None:
    """First swatch that carries PaintColorColorParam (hash 0xC0CB2820). No mask-label picking."""
    for s in swatches:
